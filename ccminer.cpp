...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
        ...
        if (pool->type & POOL_STRATUM) {
                ...
                if (work->mweb_data && work->mweb_size > 0) {
                        mweb_hex = bin2hex(work->mweb_data, work->mweb_size);
                }
                ...
                /* MWEB: Build submission with MWEB data for RINHASH */
                char *submission_data = str; // default to regular block
                if (opt_algo == ALGO_RINHASH && work->mweb_data && work->mweb_size > 0) {
                        char *mweb_block = format_mweb_block(str, work->mweb_data, work->mweb_size);
                        if (mweb_block) {
                                submission_data = mweb_block;
                                if (opt_protocol)
                                        applog(LOG_DEBUG, "MWEB: Submitting block with MWEB data (%zu bytes)", work->mweb_size);
                        } else {
                                applog(LOG_ERR, "MWEB: Failed to format block with MWEB data, using regular submission");
                        }
                }
...
// MWEB: Dynamic getblocktemplate request with MWEB support  
static char *build_gbt_req() {
    static char gbt_buffer[512];
    if (opt_algo == ALGO_RINHASH) {
