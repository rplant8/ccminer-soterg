        ...
        bool longpoll_scan, bool longpoll, int *curl_err)
{
        ...
        size_t max_user_len = sizeof(userpass) / 2 - 2;
        size_t max_pass_len = sizeof(userpass) / 2 - 2;
        userpass[0] = '\0';

        // Sao chép user, giới hạn độ dài
        strncat(userpass, pool->user, max_user_len);

        // Nếu có password thì nối thêm dấu ':' và password, giới hạn độ dài
        if (strlen(pool->pass) > 0 && strlen(userpass) < sizeof(userpass)-2) {
            strncat(userpass, ":", 1);
            strncat(userpass, pool->pass, max_pass_len);
        }
...
void rinhash_set_target_ratio(struct work* work, uint32_t* hash) {
    ...
    if (work->submit_nonce_id < 0 || work->submit_nonce_id >= MAX_NONCES) return;
